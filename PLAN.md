...
### P_C.6 — Suppression de la pré-lecture des durées post-collecte

**Constat.** `UI/MainForm.cs:599-677` (`ReadHeaderDurationsAsync`) ouvre chaque fichier juste après la collecte pour lire 42 octets (FLAC) ou 10 Ko (MP3) et en extraire la durée, uniquement pour alimenter une colonne "Duration" pré-analyse et le compteur "Duration Total" de la status bar. Observé : 5 à 10 secondes pour 535 fichiers sur HDD. L'opération est séquentielle, non parallélisée, et ajoute un `CreateFile` + seek + read + close par fichier sur le chemin critique.

**Aggravation.** L'opération est lancée en fire-and-forget avec `_scanCts.Token`, qui n'est pas annulé quand l'utilisateur clique "Start scan". La pré-lecture continue donc pendant l'analyse, provoquant des seeks parasites sur HDD qui se superposent aux lectures de l'AnalysisPipeline.

**Double lecture.** Les checkers (`NativeFlacChecker.cs:200`, `Mp3Checker.cs`) recalculent eux-mêmes la durée depuis le buffer déjà chargé en RAM pendant l'analyse. La pré-lecture est donc doublement gaspillée : elle paye des I/O qui seront refaites.

**Solution.**

- Supprimer complètement `ReadHeaderDurationsAsync` et son appel depuis `ScanAsync`.
- Supprimer l'indicateur "Duration Total" du status bar (retrait volontaire, esthétique sans valeur fonctionnelle après cette suppression).
- Conserver la colonne "Duration" de la ListView, qui sera peuplée au fil de l'eau par `OnFileCompleted` pendant le scan d'intégrité (la durée vient alors gratuitement du checker, déjà calculée sur le buffer RAM).
- Nettoyer les champs et méthodes associés (`_totalScannedDuration`, `_totalDuration`, `FormatTrackDuration` si plus utilisé).

**Impact matériel.** Gain massif sur HDD (5-10 s économisés par queue de 500 fichiers), mesurable sur SATA SSD, marginal sur NVMe. Bonus : suppression des seeks parasites pendant le scan d'intégrité.

**Test.** Déposer une queue de 500+ fichiers sur HDD, vérifier que l'affichage est instantané après la collecte. Lancer le scan, vérifier que la colonne Duration se remplit au fil des complétions. Confirmer que le status bar n'affiche plus Duration Total.

**Risque.** Très faible. Suppression nette, aucune nouvelle logique. Les champs nettoyés sont locaux à `MainForm`.
...
### Phase 3, nettoyage UI et fenêtre d'options

10. P_C.6 — suppression de `ReadHeaderDurationsAsync` et de l'indicateur "Duration Total" du status bar.
11. Création de `UI/OptionsForm.cs` avec catégorie Performances.
12. Ajout du menu Tools dans `MainForm.cs`.
13. Extension `UserPreferences.cs`.
14. Catégorie Tools de `OptionsForm` avec détection live.
15. Indicateur status bar "Storage: …".
